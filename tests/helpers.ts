...
// Generate a sample graph of the named preset in the app itself — the path a
// device with no data on it takes.
export async function generateSample(page: Page, preset: string): Promise<void> {
  await page.getByTestId(`generate-${preset}`).click();
}

// As `ingestAndLayout`, but the graph is synthesized in the tab instead of
// dropped, so the hash the two return is comparable.
export async function generateAndLayout(page: Page, preset: string): Promise<string> {
  await page.goto('/');
  await generateSample(page, preset);
