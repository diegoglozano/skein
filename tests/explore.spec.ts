...
test('search selects a node, lists its neighbours, and hover picks it back', async ({ page }) => {
...
test('picking still works after a re-layout', async ({ page }) => {
  test.setTimeout(180_000);
  page.on('pageerror', (err) => console.error('[page error]', err.message));
...
test('a query with no match reports it', async ({ page }) => {
  test.setTimeout(180_000);
  await ingestAndLayout(page, 'tiny.csv');
