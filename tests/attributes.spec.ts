...
test('attributes join onto nodes and report unmatched keys', async ({ page }) => {
...
// session that never asks for them must not pay 34 MB for the option.
...
test('colour by a column repaints the canvas and shows a legend', async ({ page }) => {
  test.setTimeout(300_000);
  page.on('pageerror', (err) => console.error('[page error]', err.message));
  ...
  await settle(page);
  const sequential = await canvas.screenshot();
  expect(sequential.equals(coloured), 'switching to a numeric column changed nothing').toBe(false);

  // Sizing is a separate byte of the same packed style word.
  await page.getByTestId('size-by').selectOption('degree');
  await settle(page);
  const sized = await canvas.screenshot();
  expect(sized.equals(sequential), 'size-by did not change the rendered frame').toBe(false);
});

test('a filter hides nodes, counts them, and clears back', async ({ page }) => {
  test.setTimeout(300_000);
  page.on('pageerror', (err) => console.error('[page error]', err.message));
...
test('the selection card shows the selected node attributes', async ({ page }) => {
  test.setTimeout(300_000);
  page.on('pageerror', (err) => console.error('[page error]', err.message));
...
// WebGL2 — and a bad GL draw fails silently rather than throwing.
...
// Reopening a graph must restore the join without asking for the file again:
// the bytes were persisted to OPFS next to the CSR.
test('an attached file survives closing and reopening the graph', async ({ page }) => {
  test.setTimeout(300_000);
  page.on('pageerror', (err) => console.error('[page error]', err.message));
